...
# model_weight_path = 'SL_Run_Fix_Batching_119_tmp'
...
TEST_DATA_DIR = "Problem_TestSet_GA"
...
def run_ai_solver(model, problem_file, workpieces_objs, machine_power_data, device):
    ...
    # 处理输出
    edges_matrix = generated_edges.squeeze(0).argmax(dim=-1).detach().cpu()
    priorities = priorities.squeeze(0).detach().cpu()
    ...
    end_time = time.time() # 验证也算在推理流程中，或者你可以移到上面去
    inference_time = end_time - start_time

    if not is_valid:
        # print('⚠️ Invalid graph generated.')
        print('error code 6612')
        return float('inf'), float('inf'), False, inference_time, None
    ...
    try:
        ...
        return mk, eng, True, inference_time, completed_ops
...
def main():
    ...
    gantt_dir = Path("Gantt_Charts_TestSet_RL_Trained_Diffusion")
    ...
    save_to_csv(csv_data_model, f"result_testset_{model_weight_path}_RL.csv")
