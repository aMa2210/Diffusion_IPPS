...
# --- 修改建议示例 ---
RUN_NAME = "rl_new0110_decay_scaler(TScaler0.01)_introduce_advantage_resume_from_ep999(LR1e-7)"  # 1. 改名
LR = 1e-7

# 3. 将起始值设为上一轮的结束值 (保持低熵、低Guidance强度的稳定状态)
ENTROPY_START = 0.001  # 上一轮的 ENTROPY_END
ENTROPY_END = 0.0005   # 可以更低，或者保持不变
DECAY_STEPS = 500      # 衰减步数可以保持，或者设短一点

# 同理调整 Scaler
POS_SCALER_START = 0.2 # 上一轮的 POS_SCALER_END
POS_SCALER_END = 0.2
T_SCALER_START = 0.001 # 上一轮的 T_SCALER_END
T_SCALER_END = 0.001
SEED = 65
random.seed(SEED)
...
CHECKPOINT_PATH = "rl_checkpoints/rl_new0110_decay_scaler(TScaler0.01)_introduce_advantage/model_ep999.pth" 
