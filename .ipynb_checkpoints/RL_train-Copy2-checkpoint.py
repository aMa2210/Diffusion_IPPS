...
if __name__ == '__main__':
    ...
    RUN_NAME = "rl_adding_temperature_BS32_T4_Layer6_HIDDEN_DIMENSION256_extendedTrainset_halfK_fullTrainset"
    ...
    for epoch in range(EPOCHS):
        ...
        for prob in pbar:
            ...
            canvas_cpu = prob['canvas'].clone().cpu()
            tasks = []
            for i in range(BATCH_SIZE):
                task_args = (
                    e_batch_indices[i],          # 单个样本的边矩阵
                    batch_priorities_cpu[i],     # 单个样本的优先级
                    canvas_cpu,              # 共享的画布信息
                    prob['wp_objs'],             # 共享的工件对象
                    prob['power_data']           # 共享的能耗数据
                )
            ...
            for idx, (ms, ft, err) in enumerate(results):
                if err:
                    print(f"Simulation Error in sample {idx}: {err}")
                    # 遇到错误给一个巨大的惩罚值
                    batch_makespans.append(1e6)
                    batch_flow_times.append(1e6)
                else:
                    batch_makespans.append(ms)
                    batch_flow_times.append(ft)
            
            # 转回 Numpy/Tensor 继续后面的 Loss 计算
        ...
        avg_train_loss = epoch_loss_sum / len(sampled_problems)
        avg_train_makespan = epoch_makespan_sum / len(sampled_problems)
